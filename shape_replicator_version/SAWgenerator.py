...
class WithPicture:
    ...
    def get_contour(self):
        ...
        to_remove = 0
        for i in reversed(range(0, len(self.y) - 1)):
            if self.y[0] - 10 <= self.y[i] <= self.y[0] + 10:
                to_remove += 1
            else:
                break
        self.x, self.y = self.x[:-to_remove], self.y[:-to_remove]
    ...
    def contour_bias(self):
        ...
        for i in range(1, len(self.x)):
            ...
            dx *= 1 / distance
            dy *= 1 / distance
            self.distance_list.append(round(7 * distance))
            if dy != 0 and dx != 0:
                if dx < 0 and dy < 0:
    ...
    def reached_top(self):
        if self.SAW.total_minmax_y[0] + 20 >= self.SAW.y:
            return True
        else:
            return False

    def reached_bottom(self):
        if self.SAW.total_minmax_y[1] - 20 <= self.SAW.y:
            return True
        else:
            return False

    def filling_process(self):
        self.SAW.biases = [0.49, 0.01, 0.49, 0.01]
        for i in range(5):
            self.SAW.step_forward()
        self.SAW.biases = [0.01, 0.01, 0.97, 0.01]
        for i in range(5):
            self.SAW.step_forward()
        self.SAW.biases = [0.01, 0.49, 0.49, 0.01]
        for i in range(5):
            self.SAW.step_forward()
        self.SAW.biases = [0.01, 0.97, 0.01, 0.01]
        for i in range(10):
            self.SAW.step_forward()
        self.SAW.biases = [0.01, 0.49, 0.01, 0.49]
        for i in range(5):
            self.SAW.step_forward()
        self.SAW.biases = [0.01, 0.01, 0.01, 0.97]
        for i in range(20):
            self.SAW.step_forward()
        self.SAW.biases = [0.49, 0.01, 0.01, 0.49]
        for i in range(30):
            self.SAW.step_forward()
        self.SAW.biases = [0.97, 0.01, 0.01, 0.01]
        for i in range(20):
            self.SAW.step_forward()
        self.SAW.biases = [0.49, 0.01, 0.49, 0.01]
        while not self.reached_top():
            for i in range(abs(self.SAW.total_minmax_y[0] - self.SAW.y)):
                self.SAW.step_forward()
        self.SAW.biases = [0.97, 0.01, 0.01, 0.01]
        while not self.reached_bottom():
            to_fill = abs(self.SAW.total_minmax_y[0] - self.SAW.y)
